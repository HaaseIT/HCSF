...
if (getUserData()) {
    ...
    $P->cb_customcontenttemplate = 'customer/register';
    ...
    if (isset($_POST["doRegister"]) && $_POST["doRegister"] == 'yes') {
        ...
        if (count($aErr) == 0) {
            ...
            if ($iRows == 0) {
                ...
                $P->cb_customdata["customerform"] = buildCustomerForm($C, $sLang, 'register', $aErr);
            }
        } else {
            $P->cb_customdata["customerform"] = buildCustomerForm($C, $sLang, 'register', $aErr);
        }
    } else {
        $P->cb_customdata["customerform"] = buildCustomerForm($C, $sLang, 'register');
    }
    if (isset($aPData) && count($aPData)) {
        $P->cb_customdata["register"] = $aPData;
    }
}
