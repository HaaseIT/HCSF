...
if ($C["show_pricesonlytologgedin"] && !getUserData()) {
    ...
    if (isset($_GET["msg"]) && trim($_GET["msg"]) != '') {
        if (($_GET["msg"] == 'updated' && isset($_GET["cartkey"]) && isset($_GET["amount"])) || ($_GET["msg"] == 'removed') && isset($_GET["cartkey"])) {
            $P->oPayload->cl_html .= \HaaseIT\Textcat::T("shoppingcart_msg_" . $_GET["msg"] . "_1") . ' ';
            if (isset($C["custom_order_fields"]) && mb_strpos($_GET["cartkey"], '|') !== false) {
                ...
                foreach ($mCartkeys as $sKey => $sValue) {
                    if ($sKey == 0) {
                        $P->oPayload->cl_html .= $sValue . ', ';
                    } else {
                        $TMP = explode(':', $sValue);
                        $P->oPayload->cl_html .= \HaaseIT\Textcat::T("shoppingcart_item_" . $TMP[0]) . ' ' . $TMP[1] . ', ';
                        unset($TMP);
                    }
                }
                $P->oPayload->cl_html = \HaaseIT\Tools::cutStringend($P->oPayload->cl_html, 2);
            } else {
                $P->oPayload->cl_html .= $_GET["cartkey"];
            }
            $P->oPayload->cl_html .= ' ' . \HaaseIT\Textcat::T("shoppingcart_msg_" . $_GET["msg"] . "_2");
            if ($_GET["msg"] == 'updated') {
                $P->oPayload->cl_html .= ' ' . $_GET["amount"];
            }
            $P->oPayload->cl_html .= '<br><br>';
        }
    }
    ...
    if (!isset($aShoppingcart)) {
        $P->oPayload->cl_html .= \HaaseIT\Textcat::T("shoppingcart_empty");
    } else {
        if (isset($_POST["doCheckout"]) && $_POST["doCheckout"] == 'yes') {
            if (count($aErr) == 0) {
                try {
