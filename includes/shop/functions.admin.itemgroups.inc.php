...
function admin_showGroupForm($sErr = '', $sPurpose = 'none', $aData = array()) { // no query
    ...
    $sH .= $FORM->makeText('name', \HaaseIT\Tools::getFormField('name', ((isset($aData[DB_ITEMGROUPFIELD_NAME])) ? $aData[DB_ITEMGROUPFIELD_NAME] : '')), 540, 0);
    $sH .= '</td></tr>';
    $sH .= '<tr><td class="main">Gruppennummer:<br />';
    $sH .= $FORM->makeText('no', \HaaseIT\Tools::getFormField('no', ((isset($aData[DB_ITEMGROUPFIELD_NUMBER])) ? $aData[DB_ITEMGROUPFIELD_NUMBER] : '')), 540);
    $sH .= '</tr>';
    $sH .= '<tr><td><table border="0" cellspacing="0" cellpadding="0"><tr><td class="main">Bild:<br>';
    $sH .= $FORM->makeText('img', \HaaseIT\Tools::getFormField('img', ((isset($aData[DB_ITEMGROUPFIELD_IMG])) ? $aData[DB_ITEMGROUPFIELD_IMG] : '')), 265, 0, false, 'formtext_globalsetting');
    $sH .= '</td></tr></table></td></tr>';

    if ($sPurpose == 'edit') {
        if ($aData[DB_ITEMGROUPTABLE_TEXT_PKEY] != '') {
            $sH .= '<tr><td class="main">Kurztext:<br>';
            $sH .= $FORM->makeTextarea('shorttext', \HaaseIT\Tools::getFormField('shorttext', ((isset($aData[DB_ITEMGROUPFIELD_SHORTTEXT])) ? $aData[DB_ITEMGROUPFIELD_SHORTTEXT] : '')), 540, 150);
            $sH .= '<tr><td class="main">Details:<br>';
            $sH .= $FORM->makeTextarea('details', \HaaseIT\Tools::getFormField('details', ((isset($aData[DB_ITEMGROUPFIELD_DETAILS])) ? $aData[DB_ITEMGROUPFIELD_DETAILS] : '')), 540, 150);
            $sH .= '</td></tr>';
        } else {
