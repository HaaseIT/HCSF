...
function admin_showPageAddForm($sErr = '') {
    ...
    $sH .= $FORM->makeText('pagekey', getFormfield('pagekey', ''), 400);
    $sH .= ' ';
    $sH .= $FORM->makeSubmit();
    $sH .= $FORM->closeForm();
...
function updatePage($DB, $C, $sLang) {
    if (array_search($_SERVER["REMOTE_USER"], $C["admin_users"]) !== false) {
        ...
        $sQ = buildPSUpdateQuery($aData, DB_CONTENTTABLE_BASE, 'cb_id');
        $hResult = $DB->prepare($sQ);
    ...
    if (isset($_REQUEST["textid"])) {
        ...
        $sQ = buildPSUpdateQuery($aData, DB_CONTENTTABLE_LANG);
